...
const Index = () => {
  ...
  const aboutBlock = (
    <section className="py-16 md:py-24 relative min-h-[600px] md:min-h-[700px]" style={{
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          <div className="inline-block mb-6">
            <span className="text-sm font-semibold text-primary uppercase tracking-wider bg-white/10 px-4 py-2 rounded-full backdrop-blur-sm">من نحن</span>
          </div>
          <h2 className="text-3xl md:text-4xl lg:text-6xl font-black text-white mb-6 leading-tight drop-shadow-2xl">
            {about.title}
          </h2>
          <p className="text-base md:text-lg lg:text-xl text-white/90 leading-relaxed mb-8 max-w-3xl mx-auto drop-shadow-lg">
            {about.description}
          </p>

          {/* Stats */}
          <div className="grid grid-cols-2 gap-6 md:gap-12 max-w-2xl mx-auto mb-10">
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
              <div className="text-4xl md:text-5xl font-black text-white mb-2">{about?.stats?.customers ? String(about.stats.customers) : 'N/A'}</div>
              <div className="text-sm md:text-base text-white/80 font-semibold">عميل راضي</div>
            </div>
            <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20">
              <div className="text-4xl md:text-5xl font-black text-white mb-2">{about?.stats?.products ? String(about.stats.products) : 'N/A'}</div>
              <div className="text-sm md:text-base text-white/80 font-semibold">منتج متنوع</div>
            </div>
          </div>

          <Button
            asChild
            className="bg-white hover:bg-white/90 text-slate-900 px-8 py-6 text-base font-semibold rounded-xl transition-all duration-300 hover:shadow-2xl hover:scale-105"
          >
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="flex flex-wrap items-center gap-3 mb-6">
                  {/* Live status pill */}
                  <span className={`inline-flex items-center gap-2 text-xs font-semibold px-3 py-1.5 rounded-full ${getCurrentStatus() === 'مفتوح الآن' ? 'bg-success/10 text-success border border-success/20' : 'bg-destructive/10 text-destructive border border-destructive/20'
                    }`}>
                    ...
                    branchKeys.map((key) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 ${selectedBranch === key
